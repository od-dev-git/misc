...
left outer join(
	...
	inner join(
		...
		where
			...
			and d_detail.tenantid = 'od.bhubaneswar'
			and dd.tenantid = 'od.bhubaneswar'
		group by
			d_detail.DEMANDID)egdd1 on
		egdm1.id = egdd1.demandid
	where
		date(to_timestamp(taxperiodto / 1000))< date(to_timestamp(1646094531000 / 1000))
...
where
	...
	AND (date(formattedtaxperiodto) >= date(to_timestamp(1646094531000/1000))
	AND date(formattedtaxperiodto) <= date(to_timestamp(1648751399000/1000)))
